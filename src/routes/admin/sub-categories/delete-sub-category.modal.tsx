...
import useAdminDeleteSubCategoryMutation from './queries/admin-delete-category.mutation';

const DeleteSubCategoryModal = () => {
  const { mutateAsync, isPending } = useAdminDeleteSubCategoryMutation();

  const isModalOpen = useAdminSubCategoriesStore(
    (s) => s.isDeleteSubCategoryModeOpen,
  );
  const setIsModalOpen = useAdminSubCategoriesStore(
    ...
    >
      ...
      >
        ...
        <p className="font-nunito font-black md:text-lg mt-4">
          Are you deleting the sub-category?
        </p>
